...
/// win when frontend_server resolves duplicate keys (last-wins). ORDER MATTERS:
/// de-duplicating or sorting this list inverts which value the app sees.
Future<List<String>> mergeDartDefines(
...
Future<List<String>> _readDefineFile(String path) async {
  ...
  if (p.extension(path) == '.json' || trimmed.startsWith('{')) {
    ...
    // Dart's default toString keeps JSON true/1/1.5 as bare tokens; jsonEncode
    // here would quote strings and change every emitted define.
    return decoded.entries.map((e) => '${e.key}=${e.value}').toList();
  }
  // .env style: KEY=VALUE lines, ignore blanks and # comments. The whole line
  // is kept verbatim (never re-joined around '=') so values containing '='
