...
import 'package:xcross/src/signing/macho_signer.dart';
...
Future<void> _removeIfPresent(String path) async {
  ...
  try {
    ...
    throw XcrossError('Could not remove stale signing entry "$path": $error');
  }
}
...
Never _fail(String root, String path, String reason) =>
    throw XcrossError('Bundle "${_relative(root, path)}" is invalid: $reason.');
