...
/// afterwards, so debugserver never owns the inferior's stdio and emits no `O`
/// packets for it — GDB output forwarding alone shows nothing. The VM Service
/// `Stdout`/`Stderr`/`Logging` streams are the only channel that carries it.
/// The reference Flutter debug adapter does the same on attach (see
/// `_subscribeToOutputStreams` in the DAP's `attachRequest`).
Future<void> forwardVmServiceOutput(DartVmServiceClient vm) async {
...
/// Decode a `Stdout`/`Stderr` `WriteEvent` payload, which carries base64 bytes.
///
...
String? decodeStreamWrite(Map<String, dynamic> event) {
...
/// `[loggerName] message`.
///
/// Only `valueAsString` is read. A message long enough to be truncated by the
/// VM would need a follow-up `getObject` round trip, which is not worth a
/// round trip per log line.
String? formatLogRecord(Map<String, dynamic> event) {
  if (event['logRecord'] case final Map<String, dynamic> record) {
    ...
    final parts = [
      for (final key in const ['message', 'error', 'stackTrace'])
        if (_valueAsString(record[key]) case final value? when value.isNotEmpty)
          '$prefix$value',
    ];
...
String? _valueAsString(Object? instanceRef) =>
