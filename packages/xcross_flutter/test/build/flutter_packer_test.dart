...
void main() {
  test('Windows no longer rejects native iOS plugins', () {
    final source = File(
      'packages/xcross_flutter/lib/src/build/flutter_packer.dart',
    ).readAsStringSync();
  ...
  test('uses xcross build, temp, and DevFS names', () {
    final debugBundler = File(
      'packages/xcross_flutter/lib/src/build/flutter_debug_bundler.dart',
    ).readAsStringSync();
    final packOperation = File(
      'packages/xcross_flutter/lib/src/build/flutter_pack_operation.dart',
    ).readAsStringSync();
